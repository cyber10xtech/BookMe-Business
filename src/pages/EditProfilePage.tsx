...
import { Camera, ArrowLeft, Eye, Lock, Tag, Clock } from "lucide-react";
...
const DEFAULT_HOURS: Record<string, DayHours> = Object.fromEntries(
  DAYS.map((day) => [day, { enabled: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"].includes(day), start: "09:00", end: "17:00" }])
);
...
const EditProfilePage = () => {
  ...
  const [businessHours, setBusinessHours] = useState<Record<string, DayHours>>(DEFAULT_HOURS);
  ...
  useEffect(() => {
    if (profile) {
      if (profile.business_hours) {
        setBusinessHours(profile.business_hours as Record<string, DayHours>);
      }
      setForm({
      ...
      <div className="px-5 mt-6 pb-8 space-y-4">
        ...
        <div className="bg-card rounded-2xl p-5 border border-border space-y-4">
          ...
          <div className="space-y-3">
            {DAYS.map((day) => {
              const d = businessHours[day] || { enabled: false, start: "09:00", end: "17:00" };
              return (
                <div key={day} className="flex items-center gap-3">
                  <div className="w-10 text-xs font-semibold text-foreground">{day.slice(0, 3)}</div>
                  <select
                    value={d.start}
                    onChange={(e) => setBusinessHours((prev) => ({ ...prev, [day]: { ...d, start: e.target.value } }))}
                    disabled={!d.enabled}
                    className="h-10 rounded-lg bg-secondary text-sm px-2 border-0 flex-1 disabled:opacity-40"
                  >
                    {generateTimes().map((t) => (
                  ...
                  <select
                    value={d.end}
                    onChange={(e) => setBusinessHours((prev) => ({ ...prev, [day]: { ...d, end: e.target.value } }))}
                    disabled={!d.enabled}
                    className="h-10 rounded-lg bg-secondary text-sm px-2 border-0 flex-1 disabled:opacity-40"
                  >
                    {generateTimes().map((t) => (
                  ...
                  <Switch
                    checked={d.enabled}
                    onCheckedChange={(checked) => setBusinessHours((prev) => ({ ...prev, [day]: { ...d, enabled: checked } }))}
                  />
                </div>
              );
            })}
