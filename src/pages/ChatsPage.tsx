...
interface Conversation {
  ...
  unread_count: number;
  // Whether the 48h booking messaging window is still open — see
  // src/lib/messagingWindow.ts (canMessageBooking).
  can_message: boolean;
}
...
interface ChatableClient {
  ...
  latest_completed_booking_created_at: string;
...
const ChatsPage = () => {
  ...
  const [active,        setActive]  = useState<{ convId: string; name: string; avatar: string | null; canMessage: boolean } | null>(null);
  ...
  const loadConvs = useCallback(async () => {
    ...
    // Fetch the linked booking's created_at — drives the 48h messaging
    // window, mirroring the customer app (src/lib/messagingWindow.ts).
    const bookingIds = [...new Set(convs.map((c: any) => c.booking_id).filter(Boolean))];
    const { data: bookingRows } = bookingIds.length
      ? await supabase.from("bookings").select("id, created_at").in("id", bookingIds)
      : { data: [] as any[] };
    const bookingMap: Record<string, any> = {};
    (bookingRows || []).forEach((b: any) => { bookingMap[b.id] = b; });

    setConvs(convs.map((c: any) => {
      const p = pm[c.customer_id];
      const booking = c.booking_id ? bookingMap[c.booking_id] : null;
      return {
        ...c,
        other_name: p?.full_name || p?.business_name || "Client",
        other_avatar: p?.avatar_url || null,
        unread_count: um[c.id] || 0,
  ...
  const loadChatableClients = useCallback(async () => {
    ...
    const { data: bks } = await supabase
      .from("bookings")
      .select("id, customer_id, booking_date, created_at")
    ...
    const byCustomer = new Map<string, { booking_id: string; created_at: string }>();
    for (const b of bks) {
      if (b.customer_id && !byCustomer.has(b.customer_id)) {
        byCustomer.set(b.customer_id, { booking_id: b.id, created_at: b.created_at });
      }
    }
    ...
    for (const [profileId, { booking_id, created_at }] of byCustomer.entries()) {
      ...
      clients.push({
        ...
        latest_completed_booking_created_at:  created_at,
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`msgs-biz-${user.id}`)
      ...
      // A booking auto-creates a chat_conversations row (see the customer
      // app's booking_messaging_window migration) — listen for it so a
      // newly-booked customer shows up in the Messages tab immediately,
      // without requiring the provider to leave and re-enter the page.
      .on("postgres_changes", {
        event: "INSERT", schema: "public", table: "chat_conversations",
        filter: `provider_user_id=eq.${user.id}`,
      }, () => { loadConvs(); loadChatableClients(); })
      .on("postgres_changes", {
  ...
  }, [user, loadConvs, loadChatableClients]);
  ...
  const openChat = async (client: ChatableClient) => {
    ...
    if (!convId) {
      // Conversations are unique per (customer_id, provider_id) pair, not
      // per booking — check by that pair first (most precise, and correct
      // even if this customer's conversation was originally created for a
      // different booking and later repointed).
      const { data: byPair } = await supabase
        .from("chat_conversations")
        .select("id")
        .eq("provider_id", myProf.id)
        .eq("customer_id", client.customer_profile_id)
        .maybeSingle();
      ...
      if (byPair) {
        ...
        if (created) {
          ...
          // Race condition: another insert won — look it up by the same pair.
    ...
    if (convId) {
      setActive({
        ...
        canMessage: canMessageBooking({ created_at: client.latest_completed_booking_created_at }),
      });
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {filteredConvs.map(conv => {
                  ...
                  return (
                    <button key={conv.id}
                      onClick={() => setActive({ convId: conv.id, name: conv.other_name, avatar: conv.other_avatar, canMessage: conv.can_message })}
