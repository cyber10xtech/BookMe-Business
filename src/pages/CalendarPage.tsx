        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          ...
          {booking.customer_user_id && (
            <button onClick={() => onChat(booking)}
              className="w-full flex items-center gap-3 rounded-3xl p-4 tap-scale"
              ...
              <p className="text-sm font-bold flex-1 text-left">Message {booking.customer_name || "customer"}</p>
              <span className="text-xs font-bold text-primary px-2 py-1 rounded-xl" style={{ background: "hsl(var(--background))", boxShadow: "var(--shadow-flat)" }}>Chat</span>
            </button>
          )}
...
const CalendarPage = () => {
  ...
  // architecture (same pattern as ClientsPage.tsx / Dashboard.tsx), scoped
  // to whichever booking the business tapped "Message" on.
