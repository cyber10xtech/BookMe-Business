/**
 * Authoritative Nigerian Locations Dataset
 * All 36 States + Federal Capital Territory (FCT) & their LGAs / Area Councils
 */
...
export const NIGERIA_LOCATIONS: NigerianLocationData = {
  "Abia": [
  ...
  "Akwa Ibom": [
    ...
    "Mkpat-Enin", "Nsit-Atai", "Nsit-Ibiom", "Nsit-Ubium", "Obot Akara",
  ...
  "Borno": [
    ...
    "Maiduguri", "Marte", "Mobbar", "Monguno", "Nganzai", "Shani"
  ],
  "Cross River": [
    "Abi", "Akamkpa", "Akpabuyo", "Bakassi", "Bekwarra", "Biase", "Boki",
    "Calabar Municipal", "Calabar South", "Etung", "Ikom", "Yala",
  ...
  "Ekiti": [
    ...
    "Emure", "Gbonyin", "Ido Osi", "Ijero", "Ikole", "Ikere",
  ...
  "Federal Capital Territory (FCT)": [
    ...
    "Abuja Municipal Area Council (AMAC)", "Kwali"
  ],
  "Gombe": [
  ...
  "Jigawa": [
    ...
    "Jahun", "Kafin Hausa", "Kaugama", "Kiri Kasama", "Kiyawa",
  ...
  "Ondo": [
    ...
    "Ifedore", "Ilaje", "Ile Oluji/Okeigbo", "Irele", "Okitipupa",
  ...
  "Oyo": [
    ...
    "Ibarapa North", "Iddo", "Iseyin", "Itesiwaju", "Iwajowa", "Kajola",
    "Lagelu", "Ogbomoso North", "Ogbomoso South", "Ogo Oluwa",
  ...
  "Yobe": [
    ...
    "Gulani", "Jakusko", "Karasuwa", "Machina", "Nangere", "Potiskum",
...
export const NIGERIA_STATES: string[] = Object.keys(NIGERIA_LOCATIONS);

/**
 * Helper: get array of all 36 Nigerian States + FCT
 */
...
/**
 * Helper: get array of LGAs or Area Councils for a selected State/FCT
 */
export function getLgasForState(stateName: string): string[] {
  ...
  // Exact match first
  if (NIGERIA_LOCATIONS[stateName]) {
    return NIGERIA_LOCATIONS[stateName];
  }

  // Case-insensitive match fallback
  const matchedKey = NIGERIA_STATES.find(
    (s) => s.toLowerCase() === normalizedSearch
  );

  if (matchedKey) {
    return NIGERIA_LOCATIONS[matchedKey];
  }

  // FCT alias check
  if (normalizedSearch === "fct" || normalizedSearch === "abuja") {
    return NIGERIA_LOCATIONS["Federal Capital Territory (FCT)"];
  }
...
/**
 * Helper: find matching official state name for any given text
 */
export function normalizeStateName(input?: string | null): string {
  if (!input) return "";
  const trimmed = input.trim().toLowerCase();
  if (trimmed === "fct" || trimmed === "abuja" || trimmed.includes("capital")) {
    return "Federal Capital Territory (FCT)";
  }
  const match = NIGERIA_STATES.find((s) => s.toLowerCase() === trimmed);
  return match || input;
}
